...
def av_choque_horario(lista_cromossomos, lista_todos_os_cromossomos_junto, lista_disciplinas, lista_cursos):
    logger = Logger('RPCServer')
    logger.info('Iniciando a avaliação da lista de cromossomos verificando choques de horário.')

    # Combina todas as sublistas em uma única lista
    lista_todos_os_cromossomos = [elem for sublist in lista_todos_os_cromossomos_junto if isinstance(sublist, list) for elem in sublist]

    for i, cromossomos_curso in enumerate(lista_cromossomos):
        logger.info('Verificando os cromossomos do curso: ' + lista_cursos[i]['nome'])

        for j, cromossomo in enumerate(cromossomos_curso):
            choques = 0
            cromossome_list = cromossomo['cromossome']
            ...
            for k, position in enumerate(cromossome_list):
                ...
                for cromossomo_analysis in lista_todos_os_cromossomos:
                    cromossomo_analysis_list = cromossomo_analysis.get('cromossome', [])

                    if len(cromossomo_analysis_list) > k and len(cromossome_list) > k:
                        nome_comparacao = get_professor_by_id(lista_disciplinas, cromossomo_analysis_list[k])
...
def verifica_disponibilidade(disponibilidade, numero, id_disciplina):
    dia_semana = mapear_para_dia_semana(numero, id_disciplina)
    ...
    if dia_semana in dias_semana_dict:
        ...
        disponibilidade_dia = disponibilidade[atributo_dia_semana]
